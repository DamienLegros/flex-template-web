...
import {
  ...
  confirmPayment,
  sendMessage,
} from './VoucherCheckoutPage.duck';
import { storeData, storedData, clearData } from './VoucherCheckoutPageSessionHelpers';
import css from './VoucherCheckoutPage.css';
...
export class VoucherCheckoutPageComponent extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    const title = intl.formatMessage({ id: 'VoucherCheckoutPage.title' }, { listingTitle });
      ...
      <div className={css.topbar}>
        <NamedLink className={css.home} name="LandingPage">
          <Logo
            ...
            title={intl.formatMessage({ id: 'VoucherCheckoutPage.goToLandingPage' })}
            format="mobile"
          />
          <Logo
            className={css.logoDesktop}
            alt={intl.formatMessage({ id: 'VoucherCheckoutPage.goToLandingPage' })}
            format="desktop"
          />
        </NamedLink>
      </div>
    ...
    const listingLink = (
      ...
      >
        <FormattedMessage id="VoucherCheckoutPage.errorlistingLinkText" />
      </NamedLink>
    );
    ...
    if (isChargeDisabledError) {
      initiateOrderErrorMessage = (
        <p className={css.orderError}>
          <FormattedMessage id="VoucherCheckoutPage.chargeDisabledMessage" />
        </p>
      );
      ...
      initiateOrderErrorMessage = (
        <p className={css.orderError}>
          <FormattedMessage
            id="VoucherCheckoutPage.initiateOrderStripeError"
      ...
      initiateOrderErrorMessage = (
        <p className={css.orderError}>
          <FormattedMessage id="VoucherCheckoutPage.initiateOrderError" values={{ listingLink }} />
        </p>
      );
    }
    ...
    const speculateTransactionErrorMessage = speculateTransactionError ? (
      <p className={css.speculateError}>
        <FormattedMessage id="VoucherCheckoutPage.speculateTransactionError" />
      </p>
    ) : null;
    ...
    if (isTransactionInitiateMissingStripeAccountError(speculateTransactionError)) {
      speculateErrorMessage = (
        <p className={css.orderError}>
          <FormattedMessage id="VoucherCheckoutPage.providerStripeAccountMissingError" />
        </p>
      );
    } else if (isTransactionInitiateBookingTimeNotAvailableError(speculateTransactionError)) {
      speculateErrorMessage = (
        <p className={css.orderError}>
          <FormattedMessage id="VoucherCheckoutPage.bookingTimeNotAvailableMessage" />
        </p>
      );
    } else if (isTransactionZeroPaymentError(speculateTransactionError)) {
      speculateErrorMessage = (
        <p className={css.orderError}>
          <FormattedMessage id="VoucherCheckoutPage.initiateOrderAmountTooLow" />
        </p>
      );
    } else if (speculateTransactionError) {
      speculateErrorMessage = (
        <p className={css.orderError}>
          <FormattedMessage id="VoucherCheckoutPage.speculateFailedMessage" />
        </p>
      );
    }
    ...
    const unitTranslationKey = isNightly
      ? 'VoucherCheckoutPage.perNight'
      : isDaily
      ? 'VoucherCheckoutPage.perDay'
      : 'VoucherCheckoutPage.perUnit';
            ...
            <section className={css.paymentContainer}>
              ...
              {showPaymentForm ? (
                <StripePaymentForm
                  ...
                  formId="VoucherCheckoutPagePaymentForm"
                  paymentInfo={intl.formatMessage({ id: 'VoucherCheckoutPage.paymentInfo' })}
              ...
              {isPaymentExpired ? (
                <p className={css.orderError}>
                  <FormattedMessage
                    id="VoucherCheckoutPage.paymentExpiredMessage"
                    values={{ listingLink }}
                  />
                </p>
              ) : null}
...
VoucherCheckoutPageComponent.defaultProps = {
...
VoucherCheckoutPageComponent.propTypes = {
...
const VoucherCheckoutPage = compose(
  withRouter,
  connect(
    mapStateToProps,
    mapDispatchToProps
  ...
  injectIntl
)(VoucherCheckoutPageComponent);

VoucherCheckoutPage.setInitialValues = initialValues => setInitialValues(initialValues);

VoucherCheckoutPage.displayName = 'VoucherCheckoutPage';
